...
- **Submit-time learning (v1.11.0, Fix A / Q18)**: `on_update`'s alias/mapping learning only fires on an explicit `Confirmed` field change — an operator who accepts a correct pre-filled `Suggested` match without touching it teaches nothing, so the same printed supplier name or item description returns `Suggested` forever. `api.update_ocr_import_on_submit` (the existing PI/PR/JE `on_submit` doc_event) now also runs `_learn_from_submitted_document(doc, ocr_import_name)` for every OCR Import it marks Completed: a supplier alias when the OCR Import's own `supplier_match_status` isn't already `Auto Matched`/`Confirmed` (**learns from `doc.supplier`, never the OCR Import's own `supplier`** — the operator may have changed it after the draft was created); and, per `Suggested` item row whose stripped `description_ocr` matches exactly one submitted line's stripped `description` (and is unique among the OCR Import's own rows — either ambiguity skips that row silently), an item alias + service mapping, or service-mapping-only when the line's `item_code` is the `default_item` catch-all (mirrors `on_update`'s default-item branch). `_save_supplier_alias` / `_save_item_alias` / `_save_service_mapping` were refactored into module-level `_upsert_supplier_alias` / `_upsert_item_alias` / `_upsert_service_mapping` (explicit `supplier`/`item_code`/`expense_account`/`cost_center` params, no `self`) so both callers share one upsert implementation — `on_update`'s own behaviour is unchanged. The whole submit-time learning step is wrapped in try/except → `frappe.log_error(title="OCR Submit Learning Failed")`; a failure never blocks the Completed status update, and there is no extra `frappe.db.commit()` (rides the caller's transaction). Journal Entry submits learn nothing — no `supplier`/comparable `items` in that shape. Does NOT enqueue Item Supplier learning (out of scope) and never touches an `Auto Matched`/`Confirmed` OCR row (already learned, or learned via alias already).
...
- Confidence gate (`_is_high_confidence`): supplier + every item must be alias/exact/service-mapping matched (NOT fuzzy/unmatched). Low-confidence records fall through to "Needs Review" unchanged. **Ignorable zero-value lines are skipped from the per-item loop (v1.11.0, Fix B / Q18 — see below)** — the "must have at least one item" check now requires at least one non-ignorable item; an all-zero record is unverifiable and does not auto-draft.
...
- **Zero-value lines (`_is_ignorable_zero_line`, v1.11.0 / Fix B / Q18)**: some suppliers print a genuine R0 line on every invoice (e.g. a haulier's return leg) that Gemini faithfully extracts as a second row — rate 0, amount 0, usually the `default_item` with no `expense_account` — which used to force `Needs Review` (a non-stock item without `expense_account`) AND fail the confidence gate, so operators deleted the row by hand before every create. `erpocr_integration/erpnext_ocr/doctype/ocr_import/ocr_import.py::_is_ignorable_zero_line(item)` is True only when `rate` AND `amount` are both 0, the row carries no PO/PR reference (`purchase_order_item` / `pr_detail`), and `item_code` is blank or resolves to a **non-stock** Item — a genuinely FREE **stock** item (amount 0, rate 0) still moves real inventory and is never ignorable. Applied in exactly three places, each ONLY when the record has at least one non-ignorable row (an all-ignorable record behaves exactly as before): `OCRImport._update_status` (ignorable rows don't count against matched/ready), `auto_draft._is_high_confidence` (see above), and `OCRImport.create_purchase_invoice` (the built PI drops the row — mirrors the manual delete-before-create operators already do). `create_purchase_receipt` and `create_journal_entry` are unchanged (out of scope — different risk shape, not assessed). The totals-reconciliation gate is unaffected: a zero row contributes zero to `Σ(qty × rate)` either way.
