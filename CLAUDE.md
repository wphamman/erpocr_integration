...
- **Optional `fleet_management` integration**: OCR runs **standalone or alongside `fleet_management`** — neither app imports nor depends on the other. Integration is bidirectional via ERPNext custom fields each app plants on the other's territory; both directions are runtime feature-detected so install order doesn't matter.
  ...
  - **OCR Import `fleet_vehicle` tag** (we plant on ourselves): the optional operator-set Fleet Vehicle tag on OCR Import (added in v1.1.5) lives as a *conditionally-installed Custom Field*, not a doctype JSON field. The setup is done by [install.py](erpocr_integration/install.py) `setup_optional_custom_fields()`, gated on `frappe.db.exists("DocType", "Fleet Vehicle")`, wired to both `after_install` and `after_migrate` hooks. This is the load-bearing rule for any future cross-app Link field: **a `Link → <doctype owned by an optional app>` declared in a doctype JSON breaks meta resolution on sites without that app**. Use the Custom Field install pattern instead. Read-side code uses `self.get("fleet_vehicle")` so the call site is safe when the field isn't installed.
