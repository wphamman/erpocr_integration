...
- **Jev shadow trial (`tasks/jev_shadow.py`), opt-in via `OCR Settings.enable_jev_shadow` (off by
  ...
  answer.
- **New OCR Import fields, ALL `hidden: 1` + permlevel-1 + read-only (blind to every role,
  including OCR Managers who also hold System Manager on the prod-copy bench — see Fixed
  below):** `jev_status`, `jev_supplier` (Data, not Link — a later supplier rename/delete must
  never break saving the import), `jev_choice_none`, `jev_probability`, `jev_cost_usd`,
  `jev_model`, `jev_matcher_supplier`/`jev_matcher_status` (the pre-auto-draft snapshot),
  `jev_candidate_count`, `jev_note`, `jev_run_at`. Written exclusively via
...
- **New OCR Settings fields** (collapsible "Jev shadow trial (experiment)" section):
  ...
  `jev-latest`), `jev_monthly_budget_usd` (default 2.0 — a SOFT cap, see Fixed below), and
  `jev_timeout_seconds` (default 20).
- **Guards, all skip-before-call (no API cost):** blank `supplier_name_ocr`; the monthly budget
  not set or reached; an own-company match (the printed name looks like one of our own `Company`
  records — OCR sometimes reads the buyer's letterhead instead of the supplier's); zero enabled
  Supplier candidates. Any HTTP error, timeout, or malformed/incomplete response is recorded as
  `jev_status = Error` and swallowed — never affects extraction, matching, or auto-draft.

### Fixed (Terra + Grok architect review, before landing)
- **Key-leak via an uncaught exception's dumped traceback.** Any exception outside the HTTP call
  itself (settings/doc load, budget sum, candidate generation, even `_finish`'s own
  `db.set_value`/`commit`) previously could escape `run_jev_shadow` to RQ's own handler, which
  calls `frappe.log_error(..., frappe.get_traceback(with_context=True))` — a frame-locals dump
  that could include `api_key` (not on frappe's header-redaction list) regardless of anything the
  job logs itself. Fixed with two defences: `run_jev_shadow`'s own frame never binds the key at
  all (only the tiny `_has_api_key`/`_typesafe_post` helpers call
  `settings.get_password("typesafe_api_key")`, each returning before their frame outlives the
  call); and a new outer wrapper (`_record_unexpected_failure`) catches anything the normal error
  paths don't, attempts a best-effort `jev_status = Error` write, and logs only a fixed static
  string — never `frappe.get_traceback()`, never the exception object or its `str()`.
- **Budget cap documented as SOFT, and `<= 0` now means "disabled," not "unlimited."** The check
  is read-then-act with no lock, so concurrent shadow jobs can each pass it before their cost is
  committed — accepted, at ~US$0.0002/call. A blank or zero `jev_monthly_budget_usd` now Skips
  every call rather than being treated as no cap.
- A response missing `usage` now still records `jev_status = Done` (a usable choice came back)
  but leaves `jev_cost_usd` blank/`None` (never a false `0`), with the reason in `jev_note`.

### Test plan
1014 tests (968 → +46: candidate generation, the shadow job's every gate/success/error path
(incl. explicit key-leak assertions, the never-raises backstop, missing-usage, and budget<=0),
the doctype-JSON hidden/permlevel/read-only contract, and the `gemini_process` enqueue wiring
incl. the pre-auto-draft snapshot and enqueue-failure isolation).
...
### Added
- **Auto-draft totals-reconciliation gate (Q11).** Auto-draft now skips a high-confidence match — routing it to human review with an amount-naming reason — when the extracted line rates don't reconcile with the extracted totals. A PI builds from `qty × rate`, so a **globally-discounted invoice** (the discount landed in Gemini's extracted subtotal but not the per-line rates — the schema has no discount field) systematically **over-drafts**: the live specimen `OCR-IMP-01918` (Cactus) auto-drafted **R2,654.98 against a R2,522.22 invoice** (a 5% discount, R132.76 over). The gate compares `Σ(qty × rate)` against the extracted **subtotal** (tax-exclusive rates) or the **total** (tax-inclusive rates — detected via the existing `_detect_tax_inclusive_rates`, so inclusive invoices don't false-fail by the tax amount), skipping when the deviation exceeds tolerance `max(1%, R1.00)`. Bidirectional (over- **and** under-draft). Degenerate cases (no subtotal → `total − tax` fallback; zero line sum → unverifiable) pass rather than block. **Manual creation is untouched** — the gate is auto-draft-only (ADR-0002's review-first path is the human answer to a flagged discount). Tolerance is module constants, not a new setting. Audit rides the existing `auto_draft_skipped_reason` field — diagnosing a skip is a group-by, as before.

### Changed
- **Chained match confidence — supplier-keyed item tiers cap to the supplier's confidence (Q10, ruled *cap, don't skip* by Willie 2026-07-10).** The two supplier-keyed item tiers (`Item Supplier` lookup and supplier-scoped `OCR Item Alias`) previously returned **Auto Matched** even when the parent supplier was only a fuzzy **Suggested** guess — an item shown at high confidence keyed off a guessed supplier. They now return the **min of the chain**: under a Suggested supplier those tiers display **Suggested** (the item pre-fill is kept — nothing is dropped). Confirmed / exact / manually-set suppliers behave exactly as before. Threaded consistently through both the invoice (`api._run_matching`) and delivery-note (`dn_api._run_dn_matching`) paths. No effect on auto-draft (a Suggested supplier already blocked it — now regression-asserted so the capped item statuses can't accidentally unblock anything). Non-supplier-keyed tiers (global alias, exact name/code, fuzzy) are unaffected — they resolve regardless of which supplier this is.

### Refactored
- **`_build_taxes_from_template` takes explicit args; the fleet `SimpleNamespace` proxy is deleted (Q9).** The v1.8.0 fleet Actual-VAT path (Q7a) called the invoice pipeline's tax builder through a `SimpleNamespace` faking an OCR Import — all six read-attributes were covered then, but any future attribute read would have AttributeError'd the fleet path on prod while the fully-mocked suite stayed green. The helper now takes `(tax_template, company, tax_amount, rates_include_tax)`; the inclusive-rate detector stays a doc read at the invoice call site (`_detect_tax_inclusive_rates(self)`) and is passed in, while the fleet caller passes `rates_include_tax=False` (a slip is a single amount — the same result the old `subtotal=0` proxy forced). The Actual-row injection semantics (ADR-0008: inject only into pure-Actual templates — the Cargo Compass fix) are **byte-identical** before/after, proven by the existing PI/PR/fleet integration tests through the create methods plus new direct-unit coverage of the refactored signature, and by a real-bench smoke (driver-dev: percentage PI net 1000/grand 1150; Actual PI VAT 150 injected; fleet Direct-Expense PI net 500/grand 575 — all rolled back).

### Fixed
- **Q11 gate is tax-inclusivity-aware** (code-review catch during this release). An earlier cut of the gate compared the line sum against the tax-exclusive subtotal unconditionally, which false-failed every legitimately **tax-inclusive** invoice (rates already include VAT — a first-class path the PI builder honours via `included_in_print_rate`) by the full tax amount, silently disabling auto-draft for that whole class. The gate now reconciles inclusive rates against the total (see Added above). Fail-safe direction (it only ever skipped to review), but the feature is now correct for inclusive suppliers.
